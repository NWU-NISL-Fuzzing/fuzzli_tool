...
def update_real_bugdict(bug_type, output_id):
    if bug_type in real_bug:
        real_bug[bug_type].append(output_id)
    else:
        real_bug[bug_type] = [output_id]
...
def classify(testcase, outputs, output_id, testbed_id, bug_type):
    ...
    false_positive_features = [ "Math.random", "Date", "setTimeout", "clearTimeout", "setInterval",
                                "catch {", "global.", ", eval)(", "this.assert", ".name"]
    for false_positive_feature in false_positive_features:
    ...
    if "//regex method mutation" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs admit an invalid regular expression", output_id)
        return True
    if len(outputs) == 6 and "regexec failed" in outputs[5][5] and (testbed_id in [6, 12]):
        update_real_bugdict("mujs throw exception with a regular expression1", output_id)
        return True
    if len(outputs) == 6 and "regular expression" in outputs[5][5] and (testbed_id in [6, 12]):
        update_real_bugdict("mujs throw exception with a regular expression2", output_id)
        return True
    if testbed_id in [3, 9] and outputs[2][3] == 10:
        update_real_bugdict("jerryscript returncode 10", output_id)
        return True
    if "JSON.stringify" in testcase and testbed_id in [3, 9]:
        update_real_bugdict("jerryscript returncode 10", output_id)
        return True
    if "use strict" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs behave differently when strict mode is enabled", output_id)
        return True
    if "++ /" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs incorrectly parse '++ /'", output_id)
        return True
    if "Object.keys" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs does not box the original value", output_id)
        return True
    if "this.hasOwnProperty" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs does not support 'this.hasOwnProperty'", output_id)
        return True
    if "Object.defineProperty" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs behave differently when using Object.defineProperty", output_id)
        return True
    if "__proto__: obj.__proto__" in testcase and "Object.getOwnPropertyNames" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs throw exception", output_id)
        return True
    if "__proto__: obj.__proto__" in testcase and "Object.getOwnPropertyNames" in testcase and "[object Object]" in outputs[4][4]:
        update_real_bugdict("duktape output different result", output_id)
        return True
    if "Object.assign" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs does not transform true into a Boolean Object", output_id)
        return True
    if "config.anchor.x" in testcase and testbed_id in [2, 8]:
        update_real_bugdict("quickjs run successfully while other engines throw exception", output_id)
        return True
    if "Object.create(null)" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs does not throw exception when create a new object with null prototype", output_id)
        return True
    if outputs[3][3] == 1 and "undefined variable" in outputs[3][5] and "for (var INDEX = 0; INDEX < 1000; INDEX++)" in testcase:
        update_real_bugdict("xs optimize a function definition incorrectly", output_id)
        return True
    if "this.global" in testcase and testbed_id in [4, 10]:
        update_real_bugdict("xs defines this.global", output_id)
        return True
    if "Object.defineProperties" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs does not throw exception when transforming a wrong value to Object.defineProperties", output_id)
        return True
    if "esModule" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs throw exception when xxx", output_id)
        return True
    if "arguments[0]" in testcase and testbed_id in [1, 7]:
        update_real_bugdict("hermes does not support arguments", output_id)
        return True
    if ".apply(" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs does not check variable type when calling apply", output_id)
        return True
    if "Object.getOwnPropertyDescriptor" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs behave differently when calling getOwnPropertyDescriptor", output_id)
        return True
    if ".link" in testcase and testbed_id in [2, 8]:
        update_real_bugdict("jerryscript behave differently when calling link", output_id)
        return True
    if "JSON.stringify" in testcase and "localeCompare" in testcase and testbed_id in [4, 6, 10, 12]:
        update_real_bugdict("xs and mujs output incorrect results", output_id)
        return True
    if "eval" in testcase and testbed_id in [1, 7]:
        update_real_bugdict("hermes behave diferently when calling eval", output_id)
        return True
    if "arr.indexOf(true, Infinity)" in testcase and testbed_id in [5, 11]:
        update_real_bugdict("duktape incorrectly handle a extremely long array", output_id)
        return True
    if "a === ++a" in testcase and testbed_id in [5, 11]:
        update_real_bugdict("duktape use wrong calculation order", output_id)
        return True
    if " = Object.create(" in testcase and testbed_id in [6, 12]:
        update_real_bugdict("mujs outputs different results when calling Object.create", output_id)
        return True
...
def main():
    global false_positive, real_bug, unprocessed
