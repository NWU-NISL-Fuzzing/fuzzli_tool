...
from src.Postprocessor.type_inferer import TypeInferer
...
class CallableProcessor:
    ...
    def extract_num_of_params(self, function_body: str):
        ...
        if params.__eq__(''):
            return [0, '']
        return [params.split(',').__len__(),params]
    ...
    def generate_self_calling(self, function_body: str, no_function=False):
        ...
        transition = self.extract_num_of_params(function_body)
        
        num_of_param = transition[0]
        params = transition[1]
...
if __name__ == '__main__':
    ...
    testcase = "var NISLFuzzingFunc =  function (assert) {assert.pop();};var NISLParameter0 = [75796.050277646765378825, -236.7972288504747725, -369067.5470504556349562, 9.4621187615698641, 3031.0986789371608533, -1360226049.46538397598441383, -2.7347022056853749, 8443407293.1018703416315655, -321.7327071640735098, 134729258.36120453260173846, 76800.10444429954492884];var NISLCallingResult = NISLFuzzingFunc(NISLParameter0);print(NISLCallingResult);"
