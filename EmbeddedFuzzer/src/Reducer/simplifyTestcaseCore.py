...
def check_effective(testcase: str, before_harness_result: Result.HarnessResult):
    # Get bug information from the original test result
    before_bug_info = Result.differential_test(before_harness_result)
    
    # Run the simplified test case on the same testbeds
    ...
    # Ensure output IDs match between before and after results
    ...
    if len(before_suspicious_id_set.union(after_suspicious_id_set)) > len(before_suspicious_id):
        return [False, after_harness_result]
    
    # Verify output classes remain the same for each testbed
    ...
    # Verify bug types remain the same for each suspicious output
    ...
    # Special handling for performance issues
    ...
    if len(performance_output_id) > 0:
        # Calculate performance gaps before simplification
        ...
        # Calculate performance gaps after simplification
        ...
        # Ensure performance gaps are not reduced
        for bed, gap in before_testbeds_gap_dict.items():
            if after_testbeds_gap_dict[bed] < gap:  
                return [False, None]

    # All checks passed - simplification is effective
    return [True, None]
